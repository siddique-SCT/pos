...
$categories = generateShortcuts($categories);
...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <div class="mb-3">
    <!-- Search Input -->
    <input type="text" id="search_product" placeholder="Search product by name or ID (Alt + 0)" class="form-control mb-2" oninput="searchProduct()">
    <!-- Shortcut Input -->
    ...
    <div class="form-check form-check-inline">
        <input class="form-check-input" type="radio" name="sortOption" id="sortByName" value="name" checked onchange="load_category_product()">
        <label class="form-check-label" for="sortByName">Sort by Name</label>
    </div>
    <div class="form-check form-check-inline">
        <input class="form-check-input" type="radio" name="sortOption" id="sortById" value="id" onchange="load_category_product()">
        <label class="form-check-label" for="sortById">Sort by ID</label>
    </div>

    <button type="button" class="btn btn-primary mb-2" onclick="load_category_product()">All (Alt + A)</button>
    <?php foreach ($categories as $category): ?>
        <button type="button" class="btn btn-primary mb-2" onclick="load_category_product('<?php echo htmlspecialchars($category['category_id']); ?>')"><?php echo htmlspecialchars($category['category_name']); ?> (Alt + <?php echo strtoupper($category['shortcut']); ?>)</button>&nbsp;&nbsp;
    <?php endforeach; ?>
</div>
            ...
            <div class="card-footer text-center">
                ...
                <div class="mt-2 text-muted">
                    <small>
                        ...
                        <?php foreach ($categories as $category): ?>
                            <b>Alt + <?php echo strtoupper($category['shortcut']); ?></b> (<?php echo htmlspecialchars($category['category_name']); ?>)<br>
                        <?php endforeach; ?>
                    </small>
                </div>
            </div>
...
let cart = [];
let total = 0;
let cur = "<?php echo htmlspecialchars($confData['currency']); ?>";
let taxPer = parseFloat("<?php echo htmlspecialchars($confData['tax_rate']); ?>");
...
document.addEventListener('keydown', function(event) {
    ...
    <?php foreach ($categories as $category): ?>
        if (event.altKey && event.key.toLowerCase() === '<?php echo strtolower($category['shortcut']); ?>') {
            event.preventDefault(); // Prevent default behavior
            load_category_product('<?php echo htmlspecialchars($category['category_id']); ?>'); // Load products for the category
        }
    <?php endforeach; ?>
});
...
function load_category_product(category_id = 0, searchQuery = '') {
    ...
    fetch('order_ajax.php', {
        ...
        body: JSON.stringify({ 
            ...
            search: searchQuery,
            sort_by: sortOption // Send the selected sorting option to the backend
        })
    })
...
function createOrder() {
    ...
    const orderData = {
        ...
        order_created_by: '<?php echo htmlspecialchars($_SESSION['user_id']); ?>',
        items: cart.map(item => ({
